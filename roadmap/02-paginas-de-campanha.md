...
> **Pré-requisito de runtime descoberto:** o `JwtStrategy` do gateway usa `process.env.JWT_SECRET || "default_secret"`, mas o gateway **não tinha `JWT_SECRET`** no `.env` → rejeitava até token admin legítimo (401). Adicionado `JWT_SECRET=access-secret` em `apps/api-gateway/.env` (mesmo valor de root/auth/products; espelha o que foi feito com `DATABASE_URL`). **⚠️ PRODUÇÃO:** o gateway em prod também precisa dessa var no ambiente de deploy, senão as escritas admin quebram (401) — ação pendente do dono (env/CI).
